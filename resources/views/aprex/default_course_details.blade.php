    ...
    const player = Plyr.setup('.js-player', { controls });
                        ...
                        <div class="col-xl-4 col-xxl-3">
                            <div class="card p-4 mb-4 bg-primary border-0 shadow-xss rounded-lg">
                                <div class="card-body">
                                    <h2 class="text-white font-xsssss fw-700 text-uppercase ls-3 ">Yearly</h2>
                                    <h1 class="display2-size text-white fw-700">₹ 9,999</h1>
                                    <h4 class="text-white fw-500 mb-4 lh-24 font-xssss">One subscription for all the courses and features</h4>
                                    <h4 class="text-white font-xssss mb-2"><i class="ti-check mr-2 text-white"></i> Unlimited Courses</h4>
                                    <h4 class="text-white font-xssss mb-2"><i class="ti-check mr-2 text-white"></i> Tests & Assesments</h4>
                                    <h4 class="text-white font-xssss mb-4"><i class="ti-check mr-2 text-white"></i> Mini Projects</h4>
                                    <h4 class="text-white font-xssss mb-4"><i class="ti-check mr-2 text-white"></i> Cirtifications</h4>
                                    ...
                                    @php
                                        ...
                                        $enrolled=Enrolled_course::where('student_id',session('rexkod_user_id'))
                                                                ->where('course_id',$data['courses']->id)
                                                                ->first();
                                    @endphp
                                    @php
                                ...
                                @endphp
                                @if (!$subscribed)
                                <a href="/aprex/payment" class="btn btn-block border-0 w-100 bg-white p-3 text-primary fw-600 rounded-lg d-inline-block font-xssss btn-light">Subscribe</a>
                                @else
                                @if (!empty($enrolled))
                                    <a href="/aprex/default_live_stream/{{ $data['courses']->id }}/0" class="btn btn-block border-0 w-100 bg-white p-3 text-primary fw-600 rounded-lg d-inline-block font-xssss btn-light">Go to Course</a>
                                @else
                                    <form action="/aprex/start_course" method="POST">
                                        @csrf
                                        <input type="hidden" name="course_id" value="{{ $data['courses']->id }}">
                                        <button type="submit" class="btn btn-block border-0 w-100 bg-white p-3 text-primary fw-600 rounded-lg d-inline-block font-xssss btn-light">Start Course</button>
                                    </form>
                                @endif
                                @endif
