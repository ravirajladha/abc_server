    ...
    const player = Plyr.setup('.js-player', { controls });
                        ...
                        <div class="col-xl-4 col-xxl-3">
                            <div class="card p-4 mb-4 bg-primary border-0 shadow-xss rounded-lg">
                                ...
                                @endphp
                                @if (!$subscribed)
                                <a href="/payment/{{ $data['courses']->id }}" class="btn btn-block border-0 w-100 bg-white p-3 text-primary fw-600 rounded-lg d-inline-block font-xssss btn-light">Subscribe</a>
                                @else
                                @if (!empty($enrolled))
                                    <a href="/default_live_stream/{{ $data['courses']->id }}/0" class="btn btn-block border-0 w-100 bg-white p-3 text-primary fw-600 rounded-lg d-inline-block font-xssss btn-light">Go to Course</a>
                                @else
                                    <form action="/start_course" method="POST">
                                        @csrf
