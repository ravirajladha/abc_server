            ...
            <div class="middle-sidebar-bottom bg-lightblue theme-dark-bg">
                <div class="middle-sidebar-left">
                    <div class="middle-wrap">
                        ...
                        {{-- school student --}}
                        @if (session('rexkod_user_type')==='school_student')
                        ...
                        <div class="card w-100 border-0 bg-white shadow-xs p-0 mb-4">
                            ...
                            <div class="card-body p-lg-5 p-4 w-100 border-0">
                                <h2 class="fw-400 font-lg d-block">Connect <b> Parent</b> </h2>
                                @if ($user->parent_id)
                                    @php
                                        $parent = User::where('id', $user->parent_id)->first();
                                    @endphp

                                    <h4>Parent name: <span>{{$parent->name}} </span></h4>
                                    <h4>Parent Code: <span>{{$parent->parent_code}} </span></h4>
                                @else
                                <form action="/connect_parent" method="POST">
                                    @csrf
                                <div class="row">

                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <label class="mont-font fw-600 font-xsss">Unique Parent Code</label>
                                            <input type="text" name="parent_code" class="form-control" placeholder="Enter Unique Parent id" value="">
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <button  type="submit" class="btn bg-current text-center text-white font-xsss fw-600 p-3 w175 rounded-lg d-inline-block border-0">Submit</button>
